...
public class FetchCreditCardDetailsTest {
	...
	public FetchCreditCardDetailsTest(String cardNumber, CreditCardType creditCardType) {
	...
	public static Collection<Object[]> data() {
		return SampleTestData.getSampleCreditCards();
	}
	...
	public void allParametersSane() {
		for (final CreditCardType creditCardType : CreditCardType.values()) {
			...
			assertTrue(String.format("%s: Min card length %s too small", creditCardType, minCardLength),
					minCardLength >= MIN_MIN_CARD_LENGTH);

			final int maxCardLength = creditCardType.getMaxCardLength();
			assertTrue(String.format("%s: Max card length %s too large", creditCardType, maxCardLength),
					maxCardLength <= MAX_MAX_CARD_LENGTH);

			assertTrue(String.format("%s: Min card length %s greater than its max %s", creditCardType, minCardLength,
					maxCardLength), minCardLength <= maxCardLength);

			final int securityCodeLength = creditCardType.getSecurityCodeLength();
			assertTrue(String.format("%s: Unusual security code length %s", creditCardType, securityCodeLength),
					securityCodeLength >= MIN_SECURITY_CODE_LENGTH && securityCodeLength <= MAX_SECURITY_CODE_LENGTH);
			...
			if (creditCardType != CreditCardType.UNKNOWN && creditCardType != CreditCardType.EMPTY) {
				...
				assertTrue(String.format("%s: Pattern must start with ^", creditCardType), regex.startsWith("^"));
			}
		}
	}
	...
	public void testForIncrementingNumbers() {
		for (int i = 4; i < this.cardNumber.length(); i++) {
			...
			final CreditCardType[] actualTypes = FetchCreditCardDetails.forCardNumber(incrementingCardNumber);
			assertTrue(String.format("CreditCardType.forCardNumber failed for %s", cardNumber),
					Arrays.asList(actualTypes).contains(creditCardType));
		}
	}

	@Test
	public void testForCardNumber() {
		final CreditCardType[] actualTypes = FetchCreditCardDetails.forCardNumber(cardNumber);
